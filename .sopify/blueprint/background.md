...
GitHub Pages 样例报告和 `uvx evidentloop demo` 是可跳过的体验入口。正式主链是安装 CLI 与同仓库 Skill，在满足能力契约的 AI host 中说“用 EvidentLoop 审计本地改动”，再打开 `audit.html` 查看 finding；需要修订时点击“复制给 AI 更新报告”，Skill 定位来源并让 runtime 在同一路径提交新 revision。`audit.json` 同步生成，供追溯和集成使用。用户项目无需复制集成说明或中间契约。
...
- `audit-feedback.jsonl`：用户可选下载的结构化裁定；报告主入口复制同一 JSONL 机器块，runtime 用它生成可回链的新 revision。
...
- 一期不做 hosted dashboard、SVG 产品 renderer 或 Markdown renderer；反馈修订只做确定性裁定重放，不触发模型复审、自动修复或跨工作区报告服务。
