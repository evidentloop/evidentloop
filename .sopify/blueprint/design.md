...
跨 artifact profile 的稳定路径分为两层：各 profile 先把异构审查目标及其上下文、证据归一化为 artifact-general 审查契约（当前为 `ReviewPack` / `ReviewResult`），复用隔离语义审查内核；再按 profile 完成可信锚定、验证、正式结论、适用时的评分和确定性呈现。当前类名和字段可以随真实 profile 演进，不把 code-diff 形态冻结为所有产物的永久输入结构。
