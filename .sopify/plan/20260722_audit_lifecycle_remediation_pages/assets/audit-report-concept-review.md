...
6. finding 卡采用单一纵向结构：为什么是问题 → 只读裁定摘要 → 全宽 diff → 下方全宽操作区。
7. 评论与严重度展开只向下增加高度，不改变 diff 宽度，不使用覆盖层、悬浮栏或侧栏。
8. 代码长行不折行；diff 在有最大高度的容器内按需上下/左右滚动；页面本身不横向溢出。
9. 375px 下 truth grid 为 2×2，裁定摘要为 2+1，finding 标题全宽，状态徽标独立，裁定与导出按钮成对并排。
10. 没有 fix verification、finding、feedback history 或 fixes 时，对应 section 连标题和空框一起省略。
...
- 删除 `risk_score`、`risk_delta`、`model_risk_score`、`unscored_finding_count`、`extensions.evidentloop.unscored` 以及只为是否计分存在的公开 view 字段。保留 `risk` finding category、finding severity、`pack_completeness`、downgrade provenance 与证据锚点事实。
...
- claim 真值表固定为：`supported`=support only、`challenged`=challenge only、`partial`=support+challenge、`unknown`=无两类 edge 且有明确 rationale。当前完整 diff 的 verdict 与 overall severity 独立计算。
...
- `finding_view`：model、human、current、feedback events、可信 hunk view；不含 `counted_in_risk`。
- `run_view`：仅当前 diff 的权威 run 顺序、人类动作摘要、前后 verdict/overall severity/open count 和报告身份；没有 feedback revision 时不生成历史 section。
...
- hash、schema、run id 集中到默认折叠的“报告身份与校验信息”，保留完整值和复制能力。
