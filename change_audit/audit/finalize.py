...
from change_audit.renderers.html import AuditRenderError, render_audit_file
...
from change_audit.validation import AuditValidationError, assert_valid_audit
...
_RUN_ID_RE = re.compile(r"(?m)^<!-- change-audit-run-id: ([^\s]+) -->\s*$")
...
def _staging_path(final_dir: Path) -> Path:
    return final_dir.parent / f".{final_dir.name}.change-audit-staging"
...
def _file_nodes(bundle: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(bundle.files, start=1):
        ...
        if item.old_path and item.new_path and item.old_path != item.new_path:
            node["extensions"] = {
                "change_audit": {"old_path": item.old_path, "new_path": item.new_path}
            }
        nodes.append(node)
    return nodes
...
def _validate_run_identity(raw_analysis: str, expected_run_id: str) -> None:
    ...
    if identities != [expected_run_id]:
        raise AuditWorkflowError(
            "raw analysis must contain exactly one matching change-audit run identity"
        )
