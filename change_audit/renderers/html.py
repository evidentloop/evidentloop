...
def _build_context(audit: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for finding in finding_nodes:
        ...
        extension = finding.get("extensions", {}).get("change_audit", {})
        hunk_view = (
            _as_hunk_view(
...
def render_audit_data(audit: Mapping[str, Any]) -> str:
    ...
    try:
        ...
        context["trusted_css"] = _resource_text(
            "change_audit.renderers", "static/audit.css"
        )
        context["trusted_js"] = _resource_text(
            "change_audit.renderers", "static/audit.js"
        )
        template_source = _resource_text(
            "change_audit.renderers", "templates/audit.html.j2"
        )
