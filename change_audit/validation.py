...
def load_audit_schema() -> dict[str, Any]:
    ...
    resource = files("change_audit.schemas").joinpath("audit-v0.schema.json")
...
def _validate_summary(
    ...
    unscored = [
        ...
        and isinstance(node["extensions"].get("change_audit"), Mapping)
        and (
            node["extensions"]["change_audit"].get("downgraded_from") == "bug"
            or node["extensions"]["change_audit"].get("unscored") is True
        )
    ]
