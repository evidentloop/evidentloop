...
- 当前 source candidate package version 为 `0.1.0a2`；
...
成功结果同时返回 `diff_version` 与 `report_version`。`diff_version` 与正式 JSON 的 `extensions.evidentloop.diff_version` 一致；`report_version` 对应正式 `audit.json` 原始字节，不写回 JSON 自身。
...
成功结果返回新 revision 的 `report_version`，并继承正式报告中的 `diff_version`。旧 schema `0.4` 报告没有该扩展时仍可修订，但明确返回 `diff_version: null`。
...
`finalize_review` 与 `revise_audit` 的成功结果包含 `diff_version / report_version`，供工作流在不理解内部 ReviewPack 的前提下绑定审计输入和正式报告版本。
