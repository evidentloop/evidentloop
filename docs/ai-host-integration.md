...
2. 使用同一个 Python interpreter 执行兼容探针、`prepare` 和 `finalize`；
3. 解析 locator JSON，并只使用其中返回的路径；
4. 创建不继承开发对话、shell、网络、凭据和业务写权限的隔离审查上下文；
...
|---|---|---|---|
| Codex | 已验证 | 已验证 | 尚未执行，留待 Wave 4 |
| 其他宿主 | 未验证 | 未验证 | 未验证 |

Codex 验证环境为 macOS、Codex CLI `0.144.1` 和 skills CLI `1.5.16`。验证使用隔离 HOME；安装目录为 `$HOME/.agents/skills/evidentloop`，Codex 将 Skill 识别为启用的用户级 Skill，且没有解析错误。该结果只说明当前环境的安装与发现兼容，不扩大产品支持范围。
...
没有真实、不可变、维护者发布且完成验证的 release 前，不提供外部 EvidentLoop 安装命令，不使用移动分支，也不假设 PyPI 项目存在。真实审计 E2E 与其他宿主支持继续按实测结果更新矩阵。
