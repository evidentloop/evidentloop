# EvidentLoop `0.3` code-diff 数据模型

## 契约边界

本文定义首个 `code_diff` audit profile 的 `audit.json 0.3`。它是该 profile 的正式结构化记录，结构由 JSON Schema 2020-12 定义；Python 另外执行跨对象引用、hunk anchor 和渲染回链校验。`0.3` 沿用已经验证的 code-diff 图与状态语义，并把 public identity、canonical schema URI 和 extension namespace 统一到 EvidentLoop。它不是所有 artifact 类型的预先通用 schema。

ReviewPack、ReviewResult、staging、`.run/` 和宿主 raw analysis 是内部审查传输，不属于 `audit.json` 公共契约。Renderer 不读取这些中间产物。未来非 diff profile 通过独立 ADR 和 schema 版本增加 review target 与对应 anchor，不在 `0.3` 中预塞空字段。

Canonical schema URI 为 `https://evidentloop.github.io/evidentloop/schemas/audit-v0.3.schema.json`。该 URL 是契约标识，不代表公开 Pages 已经启用。
