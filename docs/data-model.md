...
`file` 节点必选字段：`id`、`type`、`path`。

可选字段：

| 字段 | 说明 |
| --- | --- |
| `role` | 文件角色，如 `implementation`、`test`、`config` |
| `additions` | 新增行数，由 Git diff adapter 提取 |
| `deletions` | 删除行数，由 Git diff adapter 提取 |
| `change_type` | 变更类型：`added`、`modified`、`deleted`、`renamed` |

`additions` / `deletions` 缺失时 renderer 不展示 diff stat，不自行计算。
...
`audit.html` 可以额外提供面向 LLM 的复制摘要，例如把 run、verdict、finding、fingerprint、action 和用户评论整理成 Markdown。该摘要是 UI 派生视图，不是新的 `audit.json` 字段，也不是新的节点类型；机器归档格式仍以 `audit-feedback.jsonl` 为准。
...
LLM 或 review 工具不应直接生成最终 `audit.html`。它们只应生成或辅助生成符合 schema 的 `audit.json` 数据；页面结构、CSS、JavaScript、反馈导出、复制摘要和回链校验属于 renderer 的固定职责。
...
`audit.html` 渲染完成后应通过以下确定性校验：

| 校验项 | 规则 |
| --- | --- |
| `data-node-id` | 值必须存在于 `audit.json` 的 `nodes[].id` 或 `runs[].id` |
| `data-edge-id` | 值必须存在于 `audit.json` 的 `edges[].id` |
| `data-claim-id` | 值必须存在于所属 run/change 的 `summary_audit.claims[].id` |
| `data-fingerprint` | 值必须与对应 finding 节点的 `fingerprint` 字段一致 |
| `data-feedback-for` | 值必须匹配一个 `type: "finding"` 的节点 id |

校验不通过时 renderer 应报错或降级输出，不应静默生成断裂的 HTML。
