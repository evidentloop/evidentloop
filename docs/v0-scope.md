...
`review` 是 Skill 动作，不是 Python 命令。正式 console script 尚未实现。`prepare` 成功时通过 stdout 返回 locator JSON；Skill 不自行推导输出目录或 staging 路径。
...
- 反馈消费、自动修复和复杂多轮 diff matching。
- Hosted dashboard、产品 SVG renderer 或 Markdown renderer。
- PyPI 发布、release tag、console script、`demo`、`doctor` 或公开 Pages。
