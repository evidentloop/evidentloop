...
EvidentLoop v0 审计本地 Git diff：AI host 的 LLM 负责语义判断，Python 负责 Git 解析、可信锚定、状态、评分、校验和渲染。正式产物是 schema `0.3` `audit.json` 与自包含 `audit.html`。
...
- 远程发布形式的标准 Skill 安装，以及未经真实 E2E 验证的宿主支持承诺。
