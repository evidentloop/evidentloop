...
EvidentLoop v0 审计本地 Git diff：AI host 中的隔离审查者负责语义判断，Python 负责 Git 解析、可信锚定、状态、评分、校验和渲染。正式产物是 schema `0.3` `audit.json` 与自包含 `audit.html`。
...
- **Feedback loop**：HTML 可以记录并导出用户判断；当前 Alpha 尚不消费反馈或重新生成代码。
...
用户：“帮我用 EvidentLoop 审计最近的本地改动”
  ...
  -> host reviewer 在隔离上下文中分析语义问题
  -> finalize ingest ReviewResult，并用可信 hunk index 核对锚点
  -> 生成和校验 audit.json + audit.html
  -> 复查最终目标不存在
  -> 用同文件系统目录 rename 成对提交
  -> Skill 返回摘要与报告路径
  -> 用户在 HTML 中查看、判断并可选导出反馈
```

Python package 当前通过 `evidentloop` console script 提供五个命令；`python -m evidentloop` 与其行为一致：
...
`review` 是 Skill 动作，不是 Python 命令。console script 当前可从本地 checkout 安装，尚未发布到 PyPI。`prepare` 成功时通过 stdout 返回 locator JSON；Skill 不自行推导输出目录或 staging 路径。
...
用户在 HTML 中导出决策后可得到 `audit-feedback.jsonl`。浏览器下载位置不保证等于审计目录。
...
- Browser-local 决策与合法 JSONL 导出。
- `evidentloop` console script、公开 runtime/resource 诊断和离线合成 replay demo。
- 简体中文报告 UI 与 reviewer 语义正文；机器枚举、代码和路径保持原值。
- 本地 checkout 形式的 Agent Skill。
...
- 首个公开 Alpha 的反馈消费、自动修复和复杂多轮 diff matching；当前只记录并导出反馈。
- Hosted dashboard、产品 SVG renderer 或 Markdown renderer。
- PyPI 发布、release tag 或公开 Pages。
- 标准 skills CLI 安装和跨宿主支持承诺。
...
浏览器使用 `graph_id + run_id + fingerprint` 隔离本地决策，支持接受、误报、评论与严重度调整，并导出 `audit-feedback.jsonl`。当前 Alpha 不读取该文件，也不据此改代码；反馈消费与重新审查是后续闭环能力。

## 安全与质量门禁

- Diff、源码、路径、注释、intent、context 和 raw analysis 全部视为不可信数据。
- Prompt 使用运行级动态边界并冻结 source、version、完整文本 hash 与 `evidentloop-run-id`。
- 宿主 reviewer 只读，不执行 shell、不访问网络或凭据。
...
身份迁移前生成的 schema `0.2` example 保持字节不变，只作为历史证据，不要求当前 schema `0.3` renderer 兼容。
